...
const GuideReader = () => {
  ...
  const fetchGuide = async () => {
    ...
    const { data: resource } = await supabase
      .from("resources")
      .select("id, title, description, is_paid, price, currency")
      .eq("id", id)
      .maybeSingle();
    ...
    // Check access for paid guides
    const access = await checkAccess(user, resource, 'resource');
    setHasAccess(access);
  ...
  const navigateToChapter = (chapter: Chapter) => {
    navigate(`/guides/${id}/${chapter.slug}`);
  ...
  // Access gate — show paywall for paid guides the user hasn't purchased
  if (guide && guide.is_paid && hasAccess !== true) {
    return (
      <div className="min-h-screen pt-32 bg-deep-black text-off-white flex flex-col items-center">
        <div className="max-w-2xl mx-auto px-6 text-center mb-8">
          <h1 className="text-3xl font-heading font-bold mb-4">{guide.title}</h1>
          <p className="text-mid-gray">{guide.description}</p>
        </div>
        {hasAccess === 'error' && (
          <div className="max-w-2xl mx-auto mb-6 text-sm text-mid-gray">
  ...
  return (
    <div className="min-h-screen bg-deep-black text-off-white">
      ...
      <div className="flex pt-16 md:pt-20">
        ...
        {/* Sidebar */}
        <aside
          className={`fixed lg:sticky top-16 md:top-20 left-0 h-[calc(100vh-4rem)] md:h-[calc(100vh-5rem)] w-80 bg-near-black border-r border-dark-gray z-40 flex flex-col overflow-hidden transition-transform duration-300 ease-out ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
        >
              {/* Guide Header */}
